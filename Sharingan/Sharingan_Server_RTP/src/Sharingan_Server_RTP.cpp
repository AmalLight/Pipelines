#include "Sharingan_Server_RTP.h"
...
unsigned char* Sharingan_Server_RTP::insert_filenumber_filelen ( unsigned char* buffer , int buffer_len ,
...
void Sharingan_Server_RTP::sharePictures ( std::list<sockaddr_in>** cliaddr_list ,

	int sockfd1 , int sockfd2 , int sockfd3 , int sockfd4 , int input_bytes , std::string* md5_list )
{
	while ( true )
	{
		usleep ( Sharingan_Server_RTP::MILLISECONDS ) ;

		for ( int i1 = 1 ; i1 <= Sharingan_Server_RTP::DIMENSION ; i1 ++ )

			for ( int i2 = 1 ; i2 <= Sharingan_Server_RTP::DIMENSION ; i2 ++ )
			{
				int posx = ( ( i1 - 1 ) * wdim      ) ;
				int posy = ( ( i2 - 1 ) * hdim      ) ;
				int file = ( ( i1 - 1 ) *  dim + i2 ) ;

				std::string str_posx = std::to_string ( posx ) ;
				std::string str_posy = std::to_string ( posy ) ;
				std::string str_file = std::to_string ( file ) ;
				...
				if ( md5_list [ file-1 ] != md5_tmp ) md5_list [ file-1 ] = md5_tmp ;

				else continue ;

				// -------------------------------------------------------------------------------

				std::string file_number_str = ( file < 10  ? "0" : "" ) + str_file ;
				int         file_number_len = file_number_str.length            () ;

				int free_bytes_len = ( Sharingan_Server_RTP::MAXLINE * input_bytes ) ;
				...
				if (  fileptr !=  NULL )
				{
					...
					else if ( filelen_long <  free_bytes_len )
					{
						...
						buffer_send = Sharingan_Server_RTP::insert_filenumber_filelen ( buffer_send , free_bytes_len ,

							file_number_str , file_number_len ,
					        filelen_str ,     filelen_len
						) ;
						...
						switch  ( file )
						{
						    case 1:  sockfd = sockfd1 ; break ;
						    case 2:  sockfd = sockfd2 ; break ;
						    case 3:  sockfd = sockfd3 ; break ;
						    default: sockfd = sockfd4 ; break ;
						}

						list<sockaddr_in>* lista = cliaddr_list [ file-1 ] ;
...
bool Sharingan_Server_RTP::exists_in_list (                 sockaddr_in             el , std::string ip_src ) { return ( inet_ntoa ( el.sin_addr ) == ip_src ) ; }
int  Sharingan_Server_RTP::find_cliaddr_in_list ( std::list<sockaddr_in>* cliaddr_list , std::string ip_src )
{
	if ( ! cliaddr_list->empty () )
	{
		...
		for ( int i = 0 ; it != cliaddr_list->end () ; i ++ , it ++ )
			...
			if ( Sharingan_Server_RTP::exists_in_list ( *it , ip_src ) ) return i ;
	}
...
sockaddr_in Sharingan_Server_RTP::getServr1() { return this->servaddr1  ; }
sockaddr_in Sharingan_Server_RTP::getServr2() { return this->servaddr2  ; }
sockaddr_in Sharingan_Server_RTP::getServr3() { return this->servaddr3  ; }
sockaddr_in Sharingan_Server_RTP::getServr4() { return this->servaddr4  ; }
sockaddr_in Sharingan_Server_RTP::getServr5() { return this->servaddr5 ; }

int          Sharingan_Server_RTP::getInputByte () { return this->input_BYTE ; }
std::string* Sharingan_Server_RTP::getMd5List   () { return this->md5_list   ; }
...
int main ( int argc, char *argv[] )
{
	...
	Sharingan_Server_RTP Server_RTP = Sharingan_Server_RTP (
    ...
    sockaddr_in servaddr1 = Server_RTP.getServr1 () ;
    sockaddr_in servaddr2 = Server_RTP.getServr2 () ;
    sockaddr_in servaddr3 = Server_RTP.getServr3 () ;
    sockaddr_in servaddr4 = Server_RTP.getServr4 () ;
    sockaddr_in servaddr5 = Server_RTP.getServr5 () ;
    ...
    usleep ( Sharingan_Server_RTP::ONESECOND ) ;

    std::cout << "listening on port1 " << Sharingan_Server_RTP::PORT1 << std::endl ;
    std::cout << "listening on port2 " << Sharingan_Server_RTP::PORT2 << std::endl ;
    std::cout << "listening on port3 " << Sharingan_Server_RTP::PORT3 << std::endl ;
    std::cout << "listening on port4 " << Sharingan_Server_RTP::PORT4 << std::endl ;
    std::cout << "listening on port5 " << Sharingan_Server_RTP::PORT5 << std::endl ;
    ...
    initDemon () ; usleep ( Sharingan_Server_RTP::ONESECOND ) ;

    // -------------------------------------------------------------------------------------

    new std::thread ( Sharingan_Server_RTP::listen_connections , cliaddr_list , sockfd1 , sockfd2 , sockfd3 , sockfd4 ) ;

    usleep ( Sharingan_Server_RTP::ONESECOND ) ;

    new std::thread ( Sharingan_Server_RTP::listen_mouse , sockfd5 ) ;

    usleep ( Sharingan_Server_RTP::ONESECOND ) ;

    Sharingan_Server_RTP::sharePictures ( cliaddr_list , sockfd1 , sockfd2 , sockfd3 , sockfd4 , Server_RTP.getInputByte () , Server_RTP.getMd5List () ) ;
...
void Sharingan_Server_RTP::listen_connection ( std::list<sockaddr_in>** cliaddr_list , int index , char* buffer_request , int n ,
                                                         sockaddr_in     client_addr , socklen_t client_len )
{
	...
	{
	    int  i_it = Sharingan_Server_RTP::find_cliaddr_in_list ( lista , ip_src ) ;
	    if ( i_it < 0 )
...
void Sharingan_Server_RTP::listen_connections ( std::list<sockaddr_in>** cliaddr_list , int sockfd1 , int sockfd2 , int sockfd3 , int sockfd4 )
{
	cout << "------------------"             << endl ;
    ...
    while ( true )
    {
    	...
    	Sharingan_Server_RTP::listen_connection ( cliaddr_list , 1 , buffer_request , n , client_addr ,  client_len ) ;

    	n = recvfrom        ( sockfd2 , buffer_request , 20 , MSG_WAITALL , (sockaddr *) &client_addr , &client_len ) ;
    	Sharingan_Server_RTP::listen_connection ( cliaddr_list , 2 , buffer_request , n , client_addr ,  client_len ) ;

    	n = recvfrom        ( sockfd3 , buffer_request , 20 , MSG_WAITALL , (sockaddr *) &client_addr , &client_len ) ;
    	Sharingan_Server_RTP::listen_connection ( cliaddr_list , 3 , buffer_request , n , client_addr ,  client_len ) ;

    	n = recvfrom        ( sockfd4 , buffer_request , 20 , MSG_WAITALL , (sockaddr *) &client_addr , &client_len ) ;
    	Sharingan_Server_RTP::listen_connection ( cliaddr_list , 4 , buffer_request , n , client_addr ,  client_len ) ;
    }
}

void Sharingan_Server_RTP::listen_mouse ( int sockfd )
{
	cout << "------------------"      << endl ;
